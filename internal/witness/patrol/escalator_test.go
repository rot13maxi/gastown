...
func TestSeverityMapping(t *testing.T) {
	tests := []struct {
		...
		{EscalationHelpTriage, "medium"},
		{EscalationDirtyState, "medium"},
		{EscalationRefineryStuck, "high"},
		{EscalationTimerGate, "high"},
		{EscalationSwarmComplete, "low"},
		{EscalationUnknown, "medium"},
	}

	for _, tt := range tests {
		t.Run(string(tt.escalationType), func(t *testing.T) {
			got := SeverityFor(tt.escalationType)
			if got != tt.want {
				t.Errorf("SeverityFor(%q) = %q, want %q", tt.escalationType, got, tt.want)
			}
		})
	}
}
...
func TestGetPolecatDir(t *testing.T) {
	workDir := "/tmp/gastown"
	rigName := "gastown"
	polecatName := "furiosa"

	dir := GetPolecatDir(workDir, rigName, polecatName)
	expected := "/tmp/gastown/polecats/furiosa"

	if dir != expected {
		t.Errorf("GetPolecatDir = %q, want %q", dir, expected)
	}
}
